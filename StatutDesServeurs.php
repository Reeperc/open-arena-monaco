...
<head>
    ...
    <script>
        function getServiceStatus(serverIP, port, elementId) {
            ...
            xhr.send("server_ip=" + encodeURIComponent(serverIP) + "&port=" + encodeURIComponent(port));
        }

        function updateStatuses() {
            getServiceStatus('192.221.50.27', 27960, 'status-rouen');
            getServiceStatus('195.221.30.65', 27961, 'status-monaco');
            getServiceStatus('192.168.40.129', 27960, 'status-paris');
            getServiceStatus('192.168.20.27', 27965, 'status-mq');
        }
