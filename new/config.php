...
';
    ...
    $chemin_fichier_local = '/var/www/html/touche/q3config.cfg';

    // Écriture du texte dans le fichier local
    if (file_put_contents($chemin_fichier_local, $texte) !== false) {
        // Connexion SSH
        $connexion = ssh2_connect('192.168.1.2', 22);
        echo "fichier est bien";
        if ($connexion) {
            // Authentification SSH
            if (ssh2_auth_password($connexion, 'test', 'test')) {
                // Transfert du fichier via SCP
                $resultat = ssh2_scp_send($connexion, $chemin_fichier_local, '/home/test/.openarena/baseoa/q3config.cfg');
                if ($resultat) {
